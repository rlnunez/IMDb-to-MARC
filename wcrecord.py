...
class WorldCat:
    def get_labels(self, graph, uri, predicate_string):
    ...
    def getBibframe(self, OCLCnum,predicate):
        ...
        return self.get_labels(graph,uri,predicate)

    def OCLCScraper(self, OCLCNum):
        ...
        worldcatRecord['title'] = self.getBibframe(OCLCNum,'name')[0]
        worldcatRecord['summary'] = self.getBibframe(OCLCNum,'description')
        ...
        worldcatRecord['yearPublished'] = self.getBibframe(OCLCNum,'datePublished')
        worldcatRecord['language'] = self.getBibframe(OCLCNum,'inLanguage')
        worldcatRecord['genre'] = self.getBibframe(OCLCNum,'genre')
        worldcatRecord['contentRating'] = self.getBibframe(OCLCNum,'contentRating')
        worldcatRecord['subjects'] = self.getBibframe(OCLCNum,'about')
        worldcatRecord['alternateName'] = self.getBibframe(OCLCNum,'alternateName')
        worldcatRecord['awards'] = self.getBibframe(OCLCNum,'awards')
        worldcatRecord['series'] = self.getBibframe(OCLCNum,'isPartOf')
        ...
        #screen scraping information not in Bibframe
        ...
        for rows in datailsTable:
            if rows.xpath('th/text()')[0][:-1].lower() == 'language note':
                ...
                worldcatRecord['generalNotes'] = self.htmlbr_tolist(rows.xpath('td')[0])
